...
def swine_align(player_score, opponent_score):
    ...
    if player_score != 0 and opponent_score != 0:
        while potential_gcd <= player_score:
            if (player_score % potential_gcd == 0 and 
                    opponent_score % potential_gcd == 0):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_rolls = 1
    while num_rolls <= 10:
        average_score = make_averaged(roll_dice, trials_count)(num_rolls, dice)
...
def average_win_rate(strategy, baseline=always_roll(7)):
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if pig_pass(potential_score, opponent_score):
        return 0
    elif swine_align(potential_score, opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    """Extra turn strategy, but always roll 6 dice if rolling a 1 will trigger 
    an extra turn. Also uses 9 as cutoff instead of 8, since rolling 6 dice
    averages at about 8.7 points.
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    cutoff = 9
    num_rolls = 6
    if (pig_pass(score + 1, opponent_score) or 
            swine_align(score + 1, opponent_score)):
        return num_rolls
    else:
